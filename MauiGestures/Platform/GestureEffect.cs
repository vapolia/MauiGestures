...
internal partial class GestureEffect : PlatformEffect
{
    ...
    private ICommand? rightTapCommand;
    ...
    {
        if (args.PropertyName is not nameof(VisualElement.X)
            and not nameof(VisualElement.Y)
            and not nameof(VisualElement.Width)
            and not nameof(VisualElement.Height))
        {
            RefreshCommandsAndEvents();
        }
    ...
    private void RefreshCommandsAndEvents()
    {
        var element = Element;
        ...
        rightTapCommand = Gesture.GetRightTapCommand(element);
    ...
    private static void TriggerCommand(ICommand? command, object? parameter)
    {
        if (command?.CanExecute(parameter) == true)
            command.Execute(parameter);
    }

    private static void TriggerCommand<T>(ICommand<T>? command, T parameter)
    {
        if (command?.CanExecute(parameter) == true)
            command.Execute(parameter);
    }

    private static void TriggerEvent<T>(Action<T>? action, T args)
    {
        action?.Invoke(args);
    }
